...
export function useStoreData(filters: StoreFilters, sortBy: string) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('store-data-refresh', handleRefresh);
    return () => window.removeEventListener('store-data-refresh', handleRefresh);
  }, []);

  return { diamonds, loading, error, refetch: fetchStoreData };
}
