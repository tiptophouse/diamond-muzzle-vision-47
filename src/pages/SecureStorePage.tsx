...
import { Filter, SortAsc, AlertCircle, Search, Shield } from "lucide-react";
...
export default function SecureStorePage() {
  ...
  const [storeOwner, setStoreOwner] = useState<string | null>(null);
  const { selectionChanged, impactOccurred } = useTelegramHapticFeedback();

  // Decode the encrypted data to get store owner info
  useEffect(() => {
    if (encryptedData) {
      try {
        const decoded = JSON.parse(atob(encryptedData));
        setStoreOwner(decoded.owner || "Diamond Dealer");
      } catch (error) {
        console.error('Failed to decode store data:', error);
        setStoreOwner("Diamond Store");
      }
    }
  }, [encryptedData]);
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h1 className="text-xl font-semibold text-foreground">{storeOwner}</h1>
                      <p className="text-sm text-muted-foreground">
                        {sortedDiamonds.length} diamonds available
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="fixed bottom-4 left-4 right-4 z-40">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-center">
              <div className="flex items-center justify-center gap-2 text-blue-700 text-sm">
                ...
                <span>Secure store access • View only</span>
              </div>
            </div>
          </div>
