...
export function BulkUserAdder({ onUsersAdded }: { onUsersAdded?: () => void }) {
  ...
  const addBulkUsers = async () => {
    ...
    try {
      console.log(`🚀 Starting bulk add of ${TELEGRAM_IDS.length} users...`);
      ...
      const existingIds = new Set(existingUsers?.map(u => u.telegram_id) || []);
      const newUsers = TELEGRAM_IDS.filter(id => !existingIds.has(id));

      console.log(`📊 Found ${existingIds.size} existing users, ${newUsers.length} new users to add`);

      if (newUsers.length === 0) {
        toast({
          title: "כל המשתמשים כבר קיימים",
          description: "כל ה-Telegram IDs שסופקו כבר נמצאים במערכת",
        });
        return;
      }

      // Prepare user profiles data
      const userProfiles = newUsers.map((telegramId, index) => ({
        telegram_id: telegramId,
        first_name: `User`,
        last_name: telegramId.toString(),
        username: null,
        phone_number: null,
        is_premium: true,
        status: 'active',
      ...
      // Insert user profiles in batches
      const batchSize = 100;
      let addedCount = 0;
      ...
      for (let i = 0; i < userProfiles.length; i += batchSize) {
        ...
        if (profileError) {
          console.error('Error inserting batch:', profileError);
          throw profileError;
        }

        addedCount += batch.length;
        console.log(`✅ Added batch: ${addedCount}/${userProfiles.length}`);
      }

      // Create user analytics for all new users
      const analyticsData = newUsers.map(telegramId => ({
        telegram_id: telegramId,
        total_visits: 0,
        api_calls_count: 0,
        storage_used_mb: 0,
        cost_per_user: 0,
        revenue_per_user: 75, // Premium pricing
        profit_loss: 75,
        lifetime_value: 75,
      ...
      for (let i = 0; i < analyticsData.length; i += batchSize) {
        ...
        if (analyticsError) {
          console.warn('Error inserting analytics batch:', analyticsError);
        }
      }

      console.log(`🎉 Successfully added ${addedCount} new users!`);

      toast({
        title: "משתמשים נוספו בהצלחה!",
        description: `נוספו ${addedCount} משתמשים חדשים למערכת`,
      });
      ...
      toast({
        title: "שגיאה בהוספת משתמשים",
        description: error.message || "אירעה שגיאה בעת הוספת המשתמשים",
        variant: "destructive",
      });
    } finally {
      setIsAdding(false);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          הוסף {TELEGRAM_IDS.length} משתמשים למערכת ניהול המשתמשים
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="bg-blue-50 p-4 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-medium">פרטי ההוספה:</span>
          </div>
          <ul className="text-sm space-y-1">
            <li>• {TELEGRAM_IDS.length} Telegram IDs להוספה</li>
            <li>• כל המשתמשים יקבלו מנוי Premium</li>
            <li>• יתווסף Analytics נתונים לכל משתמש</li>
            <li>• מחיר לכל משתמש: $75</li>
          </ul>
        </div>
        ...
        >
          {isAdding ? (
            ...
            <>
              ...
              הוסף {TELEGRAM_IDS.length} משתמשים למערכת
            </>
          )}
        </Button>
      </CardContent>
