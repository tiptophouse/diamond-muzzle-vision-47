...
import { Trash2, Edit, Eye, Search, Filter, RefreshCw, Copy, Plus } from "lucide-react";
import { Diamond } from "@/components/inventory/InventoryTable";
import { Checkbox } from "@/components/ui/checkbox";

interface OptimizedInventoryDashboardProps {
  diamonds: Diamond[];
  loading: boolean;
  error?: string | null;
...
export function OptimizedInventoryDashboard({
  ...
  onBulkDelete,
  onRefresh
}: OptimizedInventoryDashboardProps) {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(filteredDiamonds.map(d => d.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const handleSelectDiamond = (diamondId: string, checked: boolean) => {
    if (checked) {
      setSelectedIds(prev => [...prev, diamondId]);
    } else {
      setSelectedIds(prev => prev.filter(id => id !== diamondId));
    }
  };

  const handleBulkAction = (action: 'edit' | 'delete') => {
    if (selectedIds.length === 0) return;
    
    if (action === 'edit' && onBulkEdit) {
      ...
      <CardContent>
        {loading ? (
          <div className="text-center py-8">
            ...
            <p className="mt-2 text-gray-600">Loading inventory with PostgreSQL...</p>
          </div>
              ...
              <thead>
                <tr className="border-b">
                  <th className="text-left p-2 w-8">
                    <Checkbox
                      checked={selectedIds.length === filteredDiamonds.length && filteredDiamonds.length > 0}
              ...
              <tbody>
                {filteredDiamonds.map(diamond => (
                  <tr key={diamond.id} className="border-b hover:bg-gray-50">
                    <td className="p-2">
                      <Checkbox
                        checked={selectedIds.includes(diamond.id)}
                    ...
                    <td className="p-2">
                      <div className="flex gap-1">
                        ...
                        {onDuplicate && (
                          <Button size="sm" variant="ghost" onClick={() => onDuplicate(diamond)}>
